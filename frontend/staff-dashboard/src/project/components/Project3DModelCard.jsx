  ...
  return (
    <div className="bg-cream-light rounded-lg p-3 flex items-center justify-between">
      <div>
        ...
        {restriction && <div className="text-xs text-red-600">Screenshots disabled</div>}
      </div>
      <div className="flex items-center gap-2">
        <button onClick={onView} className="px-3 py-1 bg-blue-600 text-white rounded">View</button>
        {canDelete && <button onClick={onDelete} className="px-3 py-1 bg-red-600 text-white rounded">Delete</button>}
      </div>
    </div>
  );
}
