...
export const ApprovedPurchaseOrders = () => {
  ...
  const mapToViewModel = (po) => {
    return {
      ...
      vendor: po.supplierId?.companyName || po.supplierId?.contactName || po.supplierId?.name || '-',
      supplierInfo: po.supplierId && typeof po.supplierId === 'object' ? {
        companyName: po.supplierId.companyName || '-',
        contactName: po.supplierId.contactName || '-',
        name: po.supplierId.name || '-',
        email: po.supplierId.email || '-',
        phone: po.supplierId.phone || po.supplierId.phoneNumber || '-',
      } : null,
              ...
              <tr>
                {[{ key: 'name', label: 'Order Name' }, { key: 'status', label: 'Status' }, { key: 'totalAmount', label: 'Total Amount' }, { key: 'updatedAt', label: 'Updated At' }].map(({ key, label }) => (
                  <th
                    ...
                    onClick={() => handleSort(key === 'projectName' ? 'projectId' : key)}
                  >
                    <div className="flex items-center">
                      {label}
