...
import React, { useState, useEffect } from "react";
import { fetchInvLocation, deleteInvLocation } from "../services/FinvLocationService.js"; 
import { Edit2, Trash2, Filter, Search, Download, MapPin, Package, Phone, User, Calendar } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { generatePDF } from "../utils/pdfGenerator.js";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-2xl font-bold mt-6 mb-10">Warehouse Locations</h1>
              <p className="text-gray-600">Manage and track your inventory locations worldwide</p>
            </div>
            <button
              className="bg-amber-900 hover:bg-amber-800 text-white font-semibold py-2 px-4 rounded shadow mt-6 mb-10" 
              onClick={() => navigate("/warehouse-manager/inventory-locations/add")}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
            <div className="bg-white rounded-lg shadow-md p-3 border-l-4 border-amber-500">
              <div className="flex items-center justify-between">
                ...
                <MapPin className="w-8 h-8 text-amber-500 opacity-80" />
              </div>
            </div>
            <div className="bg-white rounded-lg shadow-md p-3 border-l-4 border-blue-500">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-500 text-xs font-medium">Total Capacity</p>
                  <p className="text-2xl font-bold text-gray-800">
                    {inventories.reduce((sum, inv) => sum + (parseInt(inv.capacity) || 0), 0)} m³
                  </p>
                </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-3 border-l-4 border-green-500">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-500 text-xs font-medium">Active Filters</p>
                  <p className="text-2xl font-bold text-gray-800">{filteredInventories.length}</p>
                </div>
                <Filter className="w-8 h-8 text-green-500 opacity-80" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-6">
          <div className="bg-gradient-to-r from-amber-900 to-amber-800 px-6 py-4">
            <h2 className="text-l font-semibold text-white flex items-center gap-2">
                ...
                filteredInventories
                  ...
                  .map((inv) => (
                    <Marker key={inv._id} position={[inv.lat, inv.lng]} icon={redIcon}>
                      <Popup>
                        <div className="p-2">
                          <strong className="text-lg">{inv.inventoryName}</strong>
                          <br />
                          <span className="text-gray-600">{inv.inventoryAddress}</span>
                          <br />
                          <span className="text-gray-600">{inv.country}</span>
                          <br />
                          <span className="font-semibold text-amber-700">Capacity: {inv.capacity} m³</span>
                        </div>
                      </Popup>
                    </Marker>
                  ))}
        ...
        {/* Search and Filter Section */}
          <div className="mb-10 flex justify-end items-center gap-2">
            ...
            <div className="relative">
              ...
              {showFilter && (
                <div className="absolute right-0 top-full mt-2 bg-white border border-gray-200 rounded-lg shadow-xl w-48 z-50 overflow-hidden">
                  <ul className="text-sm">
                    {["all", "id", "name", "address", "country"].map((f) => (
                      <li
                        key={f}
                        className={`px-4 py-3 cursor-pointer hover:bg-amber-50 transition ${
                          filterBy === f ? "bg-amber-100 text-amber-800 font-semibold" : "text-gray-700"
                        }`}
                        onClick={() => {
                          setFilterBy(f);
                          setSearchQuery("");
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  <tr>
                    <td colSpan="9" className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-600 font-medium">Loading inventories...</p>
                      </div>
                    </td>
                  </tr>
                ) : filteredInventories.length > 0 ? (
                  filteredInventories.map((inv, index) => (
                    <tr 
                      key={inv._id} 
                      className={`hover:bg-amber-50 transition ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                    >
                      <td className="px-6 py-4 whitespace-nowrap left-0 z-30 bg-inherit">
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            title="Edit"
                          >
                            <Edit2 className="w-5 h-5 text-[#674636] hover:text-[#A67C52] transition" />
                          </button>
                          <button 
                            className="cursor-pointer"
                            ...
                            title="Delete"
                          >
                            <Trash2 className="w-5 h-5 text-[#674636] hover:text-[#A67C52] transition" />
                          </button>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm font-mono font-medium text-gray-900">{inv.inventoryId}</span>
                      </td>
                      <td className="px-6 py-4 max-w-xs">
                        <span className="text-sm font-semibold text-gray-900 break-words">{inv.inventoryName}</span>
                      </td>
                      <td className="px-6 py-4 max-w-md">
                        <div className="flex items-start gap-2">
                          <MapPin className="w-4 h-4 text-gray-400 mt-1 flex-shrink-0" />
                          <span className="text-sm text-gray-700 break-words">{inv.inventoryAddress}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-700">{inv.country}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-700">{inv.inventoryContact}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <User className="w-4 h-4 text-gray-400" />
                          <span className="text-sm text-gray-700">{inv.warehouseManagerName}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <Calendar className="w-4 h-4 text-gray-400" />
                          <span className="text-sm text-gray-700">{new Date(inv.createdAt).toLocaleString()}</span>
                        </div>
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan="9" className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-600 font-medium text-lg mb-2">No inventories found</p>
