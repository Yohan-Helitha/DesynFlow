...
export const ViewQuotationModal = ({ quotation, onClose }) => {
  ...
  const allLineItems = [
    ...(quotation.laborItems || []).map((item, idx) => ({ type: 'Labor', ...item, key: `labor-${idx}` })),
    ...(quotation.materialItems || []).map((item, idx) => ({ type: 'Material', ...item, key: `material-${idx}` })),
    ...(quotation.serviceItems || []).map((item, idx) => ({ type: 'Service', ...item, key: `service-${idx}` })),
    ...(quotation.contingencyItems || []).map((item, idx) => ({ type: 'Contingency', ...item, key: `contingency-${idx}` })),
    ...(quotation.taxes || []).map((item, idx) => ({ type: 'Tax', ...item, key: `tax-${idx}` })),
  ]
      ...
      <div className="bg-[#FFF8E8] rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto border border-[#AAB396]">
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            {/* Quotation Info */}
            <div>
              <h4 className="text-sm font-medium text-[#674636] mb-2">Quotation Information</h4>
                      ...
                      {quotation.sentToClient ? (
                        quotation.clientViewed ? (
                          quotation.clientResponse === 'Accepted' ? (
                            <>
                              <CheckCircle size={16} className="text-[#AAB396] mr-2" />
                              <span className="text-sm text-[#674636]">Accepted by client</span>
                            </>
                          ) : quotation.clientResponse === 'Rejected' ? (
                            <>
                              <X size={16} className="text-[#674636] mr-2" />
                              <span className="text-sm text-[#674636]">Rejected by client</span>
                            </>
                          ) : (
                            <>
                              <Clock size={16} className="text-[#AAB396] mr-2" />
                              <span className="text-sm text-[#AAB396]">Viewed, awaiting response</span>
                            </>
                          )
                        ) : (
                          <>
                            <Send size={16} className="text-[#674636] mr-2" />
                            <span className="text-sm text-[#674636]">Sent, not viewed yet</span>
                          </>
                        )
                      ) : (
                        <span className="text-sm text-[#AAB396]">Not sent to client yet</span>
                      )}
                    </div>
                  </div>
                )}
          ...
          {/* Line Items */}
          <h4 className="text-sm font-medium text-[#674636] mb-2">Quotation Line Items</h4>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-[#AAB396]">
                <thead className="bg-[#AAB396]">
                  <tr>
                    <th className="px-4 py-2 text-left text-xs font-medium text-[#FFF8E8] uppercase tracking-wider">Type</th>
                    <th className="px-4 py-2 text-left text-xs font-medium text-[#FFF8E8] uppercase tracking-wider">Description</th>
                    <th className="px-4 py-2 text-right text-xs font-medium text-[#FFF8E8] uppercase tracking-wider">Quantity/Hours</th>
                    <th className="px-4 py-2 text-right text-xs font-medium text-[#FFF8E8] uppercase tracking-wider">Unit Price/Rate</th>
                    <th className="px-4 py-2 text-right text-xs font-medium text-[#FFF8E8] uppercase tracking-wider">Total</th>
                  </tr>
                </thead>
                <tbody className="bg-[#FFF8E8] divide-y divide-[#AAB396]">
                  ...
                  <tr className="bg-[#F7EED3]">
                    <td colSpan="4" className="px-4 py-2 text-sm font-medium text-right text-[#674636]">Subtotal:</td>
                    <td className="px-4 py-2 text-sm font-medium text-right text-[#674636]">${quotation.subtotal?.toLocaleString()}</td>
                  </tr>
                  <tr className="bg-[#F7EED3]">
                    <td colSpan="4" className="px-4 py-2 text-sm font-medium text-right text-[#674636]">Total Contingency:</td>
                    <td className="px-4 py-2 text-sm font-medium text-right text-[#674636]">${quotation.totalContingency?.toLocaleString()}</td>
                  </tr>
                  <tr className="bg-[#F7EED3]">
                    <td colSpan="4" className="px-4 py-2 text-sm font-medium text-right text-[#674636]">Total Tax:</td>
                    <td className="px-4 py-2 text-sm font-medium text-right text-[#674636]">${quotation.totalTax?.toLocaleString()}</td>
                  </tr>
                  <tr className="bg-[#AAB396]">
                    <td colSpan="4" className="px-4 py-2 text-sm font-bold text-right text-[#FFF8E8]">Grand Total:</td>
                    <td className="px-4 py-2 text-sm font-bold text-right text-[#FFF8E8]">${quotation.grandTotal?.toLocaleString()}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            <button className="px-4 py-2 bg-[#674636] border border-transparent rounded-md text-sm font-medium text-[#FFF8E8] hover:bg-[#AAB396] flex items-center">
